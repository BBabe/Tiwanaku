...
if bol_save:
    ...
    for i in iz:
        for j in jz:
            ...
            if not int(unique[i,j]):
                ...
                df[j][i] = df0[j][i]
            # df0_save[j][i] = str(unique[i,j]) +  df0[j][i]
    # df0_save.to_csv(path, sep = sep, index = False)#, header = False)

    # np_tmp = np.full((Ni,Nj,2), '')
    # np_tmp[:,:,0] = df0.values
    # np_tmp[:,:,1] = unique
    ...
    df0 = pd.DataFrame(np_tmp)#[:,:,0])
    # unique = np_tmp[:,:,1]
    np_tmp = np.load(path_df, allow_pickle=True)
    df = pd.DataFrame(np_tmp)#[:,:,0])

    # tmp = pd.read_csv(path, sep = sep)
    # df0 = tmp.copy()
    # unique = np.zeros((Ni,Nj))
    # for i in iz:
    #     for j in jz:
    #         df0[j][i] = df0[j][i][1:]
    #     unique[i,j] = df0[j][i][0]
##
